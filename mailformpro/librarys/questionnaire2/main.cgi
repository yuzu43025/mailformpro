...
if($config{'questionnaire2.json'} || $config{'questionnaire2.result'}){
	##
	my(@log) = &_DB($config{'questionnaire2.file'});
	while(@log > 0){
...
if($config{'questionnaire2.json'} && $_GET{'mode'} eq 'json'){
	print "Pragma: no-cache\n";
	print "Cache-Control: no-cache\n";
	print "Content-type: text/html; charset=UTF-8\n\n";
	print "$_GET{'callback'}(\[${title}\],${json})";
}
elsif($config{'questionnaire2.result'}){
